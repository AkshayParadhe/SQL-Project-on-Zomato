...
select userid, product_id, count(product_id) as Total_Purchased 
 from sales 
 where product_id = (select product_id
 from sales group by product_id 
 order by count(product_id) 
 desc limit 1) group by userid;

#5. Which product was the most popular for each customer

select userid, product_id, cnt 
 from (select *, rank() over 
 (partition by userid order by cnt desc) as rnk
...
/* 8. If buying each product generates points eg. for product1 = 5 points, product2 = 2 points,
 ...
 select userid,sum(total_zomato_points) 
  from (select e.* , amt/Points as total_Zomato_points 
  from (select d.*,Case 
  ...
  on a.product_id = b.product_id)
  as c group by userid,product_id) as d) as e) as f 
  group by userid ;
 ...
 select * from
  ...
  from (select a.*, b.price from sales a inner join product b 
  on a.product_id = b.product_id)
  as c group by userid,product_id) 
  as d) as e) as f group by product_id ) as g) as h where rnk = 1;
 
/* 9. After getting gold membership how much points each customer earned during first year ?
 Consider 5 points for 10 rs. */
 
 select * , price/2 as Points from(select c.*,d.price 
 from (select a.*,b.gold_signup_date from sales a inner join goldusers_signup b 
...
select *, case
 ...
 from (select *,rank() over( partition by userid order by created_date desc) as rnk
 from (select a.userid, a.created_date, b.gold_signup_date 
 from sales a left outer join goldusers_signup b 
 on a.userid = b.userid and created_date >=gold_signup_date ) 
 as c ) as d;
